/*********************************************************************
 ...
 $Id: files.c,v 1.23 2007/02/28 21:58:29 neuhausi Exp $
**********************************************************************/
...
{
  FILE *in;
  int n_variables, n_samples, i, len;
  VECTOR *v;
  XDATA *x;
...
read_binary_matrix(char *file_name)
{
  ...
  for (i = 0; i < x->n; i++) {	
    ...
    name[len] = '\0';
    x->rows[i] = name;
    FREAD_CALL(fread(x->data[i], sizeof(double), x->m, in), x->m, "data vector");
  }
            ...
            int n_samples)
{
  ...
  FILE *in;
  int i, j, k, seen, cnt, tlen, idx, cp;
  int *c, *m;
  int array[n_samples]; 
  ...
  if (f->n < 10) {
    ...
    printf ("\n");
    if (f->n < 10) {
      printf (" Number of Cells in DOE: %i\n", f->c);
      printf ("\n");
      printf (" Datum in each cell\n");
      printf (" ");
      print_array_i(f->cells, f->c);
      printf ("\n");
      printf (" Cell IDs\n");
      printf (" ");
      print_array_i(f->cellid, f->s);
      printf ("\n");
    }
    printf ("\n");
#endif
            ...
            int n_wells)
{
  ...
  while (fgets(line, sizeof(line), in)) {
    ...
    tok  = strtok (line, ".");
    if (w > n_wells ) {
      fprintf(stderr, "There are more well than specified in file %s\n", file_name);
      exit(1);
    } else {
      if (w > 0) {
    ...
    if (i != n_features) {
      fprintf(stderr, "Bad feature name count in CCCC file %s\n", file_name);
      exit(1);
    }
    c++;
  }
