/*********************************************************************
 ...
 $Id: ks.c,v 1.7 2007/02/28 21:58:29 neuhausi Exp $
**********************************************************************/
		...
		char **feature_name)
{
  ...
  *reference = ARRAY_ALLOC(max, char *);
  ...
  if (errflg) {
    ...
    printf("                 data file. Multiple reference wells are entered with multiple -m\n");
...
kstwo(double data1[], int n1, double data2[], int n2, double *d)
{
  ...
  int j1=0,j2=0;
  double d1,d2,dt,en1,en2,fn1=0.0,fn2=0.0;
	   ...
	   char *feature_name)
{
  ...
  if (feature_name == NULL) {
    ...
    res->variables[i] = ARRAY_ALLOC((len+1), char);  
    strncpy(res->variables[i], feature_name, len);
    res->variables[i][len] = '\0';
  }
	      ...
	      char *feature_name)
{
  ...
  for (i = 0; i < ksdata->f; i++) {
    ...
    for (j = 0; j < ksdata->w; j++) {
      kstwo(ref, c, ksdata->data[i][j], ksdata->c[j], &d);
      vr->data[j] = d;
    }
  }
